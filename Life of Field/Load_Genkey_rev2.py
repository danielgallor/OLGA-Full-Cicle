...
def load_Genkey(genkey, output_genkey, changes_dictionary):
    ...
    with open(genkey, 'r') as original_genkey, \
        ...
        for line in lines:
                ...
                if key in line:
                    if key == "PROFILEDATA":  # Exception for PROFILEDATA, as the data structure is unique for this entry and only contains the output variables
                        ...
                        for change in change_variables:
                            ...
                            try:
                                if parameter_to_find == re.sub(r'[",]', "", re.search(rf'{parameter_to_find.split("=")[0]}=[a-zA-Z0-9()"/.,\s\-]+(?![a-zA-Z]*=)', line).group()): #look for the parameter_to_find in the line (the parameter name to match on)
                                    ...
                                    if parameter_to_change.split("=")[0] in re.sub(r'"', "", line):
                                        ...
                                        line += f" {parameter_to_change}" if re.match(r'.+,$', line) else f", {parameter_to_change}"
                                        # line = line.replace(",,", ",")
                            except:
                                pass
            ...
            f.write(line) #write line to new genkey fileW
